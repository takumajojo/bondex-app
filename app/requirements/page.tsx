...
const requirements = {
  ...
  notificationPolicy: {
    ...
    emailApi: "lib/mailer.ts（SMTP優先→Resendフォールバックの共通送信）を各所から利用。集荷完了・配達完了のステータス通知は lib/agency-status-notify.ts（テンプレは lib/agency-status-email.ts）、発行依頼受付は lib/agency-notify.ts、異常系アラートは lib/ops-alert.ts。未設定時は console にフォールバック出力。",
  },
