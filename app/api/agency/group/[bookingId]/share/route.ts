...
/**
 * GET /api/agency/group/[bookingId]/share — 有効な共有リンクを取得(無ければ発行)。
 *   印刷シートのQR用。乱発を防ぐため既存の有効リンクを再利用する。→ { url, expiresAt }
 */
export async function GET(
  ...
  const r = await getOrCreateGroupShare(bid, 30, auth.agency.name)
