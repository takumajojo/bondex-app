  ...
  try {
    switch (event.type) {
      ...
      case "checkout.session.completed":
        await handleAdditionCheckout(event.data.object as Stripe.Checkout.Session)
        break
      default:
...
// 旅行中の「追加」の決済リンク (Stripe Checkout) が支払われたら清算済みにする。
async function handleAdditionCheckout(session: Stripe.Checkout.Session): Promise<void> {
  if (session.metadata?.kind !== "addition") return // 追加以外の Checkout は対象外
  if (session.payment_status !== "paid") return
  const shipmentId = session.metadata?.shipment_id
  if (!shipmentId) return
  const shipment = await getShipment(shipmentId)
  if (!shipment) {
    console.error(`[stripe/webhook] addition: shipment not found (${shipmentId})`)
    return
  }
  if (shipment.charged_at) return // 冪等: 既に清算済み
  const pi = piIdOf(session.payment_intent)
  const amount = session.amount_total ?? shipment.amount_yen ?? 0
  await recordShipmentCharge(shipmentId, pi ?? "", amount) // charged_at + PI + 金額をセット
  await sendOpsAlert({
    subject: `【追加の決済完了】${legRef(shipment)} (${shipment.agency})`,
    lines: [
      `予約: ${legRef(shipment)} / 代理店: ${shipment.agency}`,
      `金額: ¥${amount.toLocaleString()} / 決済リンクでのお支払いを確認しました。`,
    ],
    agencyEmail: null,
  })
}
