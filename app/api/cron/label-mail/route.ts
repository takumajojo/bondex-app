...
export async function GET(req: NextRequest) {
  ...
  const lock = await acquireCronLock("label-mail")
  if (!lock.ok) {
  ...
  try {
    ...
    // 2026-08-31 監査対応: PostgREST は limit 未指定だと最大1000行で「静かに」切れる。
    // 旧実装は delivered 済みの過去行まで母集合に含めて無限に成長し、しかも発送日昇順のため
    // 1000行を超えた瞬間に「一番新しい = 一番期限が近い」行から取りこぼす構造だった。
    // SQL 側で「郵送がまだ意味を持つ行」だけに絞る:
    //   - status が集荷前 (requested/pending/issued/failed)
    //   - 投函期限が設定済み (null = 旧予約でアラート対象外)
    // これで母集合は「未郵送の現役予約」だけになり、1000行に達することは実運用上ない。
    // 万一に備え limit(1000) を明示し、上限到達時は結果に truncated を立てて検知可能にする。
    const { data, error } = await sb
      ...
      .order("label_mail_due", { ascending: true })
      .limit(1000)
    ...
    await releaseCronLock("label-mail")
  }
}
