-- D: 旧ホテルへの受入キャンセル連絡タスク。適用済み (Supabase migration hotel_change_cancel_task, 2026-09-14)。
-- 変更前に該当side(guest/pickup)が連絡済み(*_hotel_notified_at 非NULL)なら old_hotel_cancel_task=true を
-- 履歴に記録する (旧伝票破棄タスクと同等)。RPC が s(更新前行) から判定して記録する。
alter table shipment_hotel_changes add column if not exists old_hotel_cancel_task boolean not null default false;
